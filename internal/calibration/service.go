...
func (s *Service) AddBatches(id string, rev int, batches []model.MeasurementBatch) (*model.CalibrationCase, []map[string]any, error) {
	...
	for i, b := range batches {
		...
		if e != nil {
			if len(prepared) > 0 {
				_, commitErr := s.cases.Update(id, func(current *model.CalibrationCase) error {
					if current.Revision != rev {
						return fmt.Errorf("revision冲突")
					}
					for _, accepted := range prepared {
						current.Batches = append(current.Batches, accepted)
						current.EnvironmentTrend = EnvironmentTrend(*current)
					}
					return nil
				})
				if commitErr != nil {
				...
				for _, accepted := range prepared {
					s.store.AppendAudit(id, "measurement_recorded", map[string]string{"batch_id": accepted.BatchID, "phase": accepted.Phase, "quality": accepted.QualityState})
				}
			}
