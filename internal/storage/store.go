...
func cloneCase(c *model.CalibrationCase) *model.CalibrationCase {
	...
	out := *c
	out.Batches = append([]model.MeasurementBatch(nil), c.Batches...)
	out.Anomalies = append([]model.Anomaly(nil), c.Anomalies...)
	out.Reviews = append([]model.ReviewDecision(nil), c.Reviews...)
	out.Remediations = append([]model.RemediationItem(nil), c.Remediations...)
	out.Baseline.GaugeWarnings = append([]string(nil), c.Baseline.GaugeWarnings...)
	out.EnvironmentTrend.TriggerBatchIDs = append([]string(nil), c.EnvironmentTrend.TriggerBatchIDs...)
