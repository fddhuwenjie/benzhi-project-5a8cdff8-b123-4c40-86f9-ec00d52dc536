...
func (s *Store) Evidence(caseID string) ([]byte, error) {
	...
	if e := os.WriteFile(filepath.Join(s.dir, "evidence-"+caseID+".json"), b, 0644); e != nil {
		return nil, fmt.Errorf("证据包写入失败: %w", e)
	}
	if e := os.WriteFile(filepath.Join(s.dir, "evidence-"+caseID+".sha256"), []byte(s.evidenceDigest[caseID]), 0644); e != nil {
		return nil, fmt.Errorf("证据摘要写入失败: %w", e)
	}
	return b, nil
}
